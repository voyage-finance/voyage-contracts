...
contract VToken is ERC20 {
    ...
    * @return the total balance of the user
    ...
    function balanceOf(address _user) public override view returns (uint256) {
        ...
        uint256 currentJuniorCumulatedBalance = calculateCumulatedBalanceInternal(_user, CoreLibrary.Tranche.JUNIOR, currentPrincipalBalance);
        uint256 currentSeniorCumulatedBalance = calculateCumulatedBalanceInternal(_user, CoreLibrary.Tranche.SENIOR, currentPrincipalBalance);
        return currentPrincipalBalance.add(currentJuniorCumulatedBalance).add(currentSeniorCumulatedBalance);
    }
    ...
    * and the new user index
    **/
    function cumulateBalanceInternal(address _user, CoreLibrary.Tranche _tranche)
        internal
        ...
        if (_tranche == CoreLibrary.Tranche.JUNIOR) {
            index = userJuniorIndexes[_user] = liquidityManager.getReserveNormalizedIncome(underlyingAssetAddress);
        } else {
            index = userSeniorIndexes[_user] = liquidityManager.getReserveNormalizedIncome(underlyingAssetAddress);
        }
    ...
    function mintOnDeposit(address _account, CoreLibrary.Tranche _tranche, uint256 _amount) external onlyLiquidityManager {
        //cumulates the balance of the user
        (,
        ,
        uint256 balanceIncrease,
        uint256 index) = cumulateBalanceInternal(_account, _tranche);
