...
describe('Buy', function () {
  it('Buy with wrong vault address should revert', async function () {
    const { crab, voyage, priceOracle, purchaseData } = await setupTestSuite();
    await priceOracle.updateTwap(crab.address, toWad(10));
    await expect(
      voyage.buyNow(crab.address, 1, voyage.address, purchaseData)
    ).to.be.revertedWith('Unauthorised()');
  });

  it('Buy with insufficient liquidity should revert', async function () {
    const { crab, owner, voyage, priceOracle, purchaseData } =
      await setupTestSuite();
    await priceOracle.updateTwap(crab.address, toWad(10));
    const vault = await voyage.getVault(owner);
    await expect(
      voyage.buyNow(crab.address, 1, vault, purchaseData)
    ).to.be.revertedWith('InsufficientLiquidity()');
  });

  it('Buy with invalid floor price should revert', async function () {
    const { crab, owner, voyage, purchaseData } = await setupTestSuite();
    const vault = await voyage.getVault(owner);
    await expect(
      voyage.buyNow(crab.address, 1, vault, purchaseData)
