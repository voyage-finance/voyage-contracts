...
task("dev:initialize-reserve", "Initializes a reserve.")
    ...
    .addOptionalParam(
        "twapTolerance",
        "The twap tolerance. Defaults to 20%.",
        TWAP_TOLERANCE,
        types.int
    )
    .setAction(async (params, hre) => {
        ...
        const [initialized, activated] = await voyage.getReserveStatus(
            collection
        );
        ...
        await voyage.setTwapTolerance(
            collection,
            ethers.BigNumber.from(twapTolerance)
        );
    ...
    - twap tolerance: ${twapTolerance / 100}%
    `);
        ...
        await voyage
            .updateProtocolFee(owner, protocolFee)
            .then((tx) => tx.wait());
