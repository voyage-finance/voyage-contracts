...
export const setupDebtTestSuite = d.createFixture(
    ...
    const healthStrategyAddress = await ethers.getContract('DefaultHealthStrategy');
    ...
    await deployments.execute('Tus', { from: owner, log: true }, 'transfer', 
      alice, ethers.BigNumber.from(1_000_000).mul(multiplier),
    )
    await deployments.execute('Tus', { from: owner, log: true }, 'transfer', 
      bob, ethers.BigNumber.from(1_000_000).mul(multiplier),
    )
